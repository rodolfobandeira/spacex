...
describe SPACEX do
  context 'DragonCapsules', vcr: { cassette_name: 'dragon_capsules/info' } do
    ...
    it 'returns Dragon Capsules info' do
      ...
      expect(subject[0]['dry_mass_kg']).to eq 4200
      expect(subject[0]['dry_mass_lb']).to eq 9300
      ...
      expect(subject[0]['heat_shield']['temp_degrees']).to eq 3000
      ...
      expect(subject[0]['launch_payload_mass']['kg']).to eq 6000
      expect(subject[0]['launch_payload_mass']['lb']).to eq 13228
      expect(subject[0]['launch_payload_vol']['cubic_meters']).to eq 25
      expect(subject[0]['launch_payload_vol']['cubic_feet']).to eq 883
      expect(subject[0]['return_payload_mass']['kg']).to eq 3000
      expect(subject[0]['return_payload_mass']['lb']).to eq 6614
      ...
      expect(subject[1]['dry_mass_kg']).to eq 6350
      expect(subject[1]['dry_mass_lb']).to eq 14000
      ...
      expect(subject[1]['heat_shield']['temp_degrees']).to eq 3000
      ...
      expect(subject[1]['thrusters'][1]['thrust']['lbf']).to eq 16000
      expect(subject[1]['launch_payload_mass']['kg']).to eq 6000
      expect(subject[1]['launch_payload_mass']['lb']).to eq 13228
      expect(subject[1]['launch_payload_vol']['cubic_meters']).to eq 25
      expect(subject[1]['launch_payload_vol']['cubic_feet']).to eq 883
      expect(subject[1]['return_payload_mass']['kg']).to eq 3000
      expect(subject[1]['return_payload_mass']['lb']).to eq 6614
