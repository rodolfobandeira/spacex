...
describe SPACEX do
  context 'Ships', vcr: { cassette_name: 'ships/info' } do
    ...
    it 'returns Ships info' do
      ...
      expect(subject.ship_model).to eq null
      ...
      expect(subject.course_deg).to eq null
      expect(subject.successful_landings).to eq null
      expect(subject.attempted_landings).to eq null
